...
.. versionchanged:: 5.0
	...
	for method in iface.names():
		...
		if not callable(candidate.__dict__[method]):
			raise Invalid("Candidate defines non-callable method",
						  candidate, iface, method)
		if not verifyMethod(iface, candidate, method):
			raise Invalid("Candidate defines method with wrong signature",
						  candidate, iface, method)
